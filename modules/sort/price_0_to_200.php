...
{
	$id_loai=$_GET['id'];
	$number_rows = db_num_rows("SELECT * FROM sanpham where maloai= $id_loai and trangthai = 1 and gia between 0 and 200");
	// echo $number_rows;	
}
else
{
...
{
	$id_loai=$_GET['id'];
	$Listproduct = get_product_0_200($start, $num_per_page,$id_loai);
}
// else
// {
 ...
 -->
                ...
                <div class="pagination flex-m flex-w p-t-26" style="margin: auto;     margin-left: 50%;">
					...
					{if (!empty($Listproduct)) {
						...
						echo get_pagging($num_page, $page, "?mod=sort&act=price_0_to_200",$id_loai);
					}
					else
					{
						echo get_pagging_main($num_page, $page, "?mod=sort&act=price_0_to_200");
					}
